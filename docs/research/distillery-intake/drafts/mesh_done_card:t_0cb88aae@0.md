# Restore failover heartbeat LaunchAgent process

```yaml
id: mesh_done_card:t_0cb88aae@0
source_kind: mesh_done_card
source_path: t_0cb88aae
source_ref: t_0cb88aae@0
board: mesh
captured_at: 2026-08-09T03:15:16Z
status: stale
status_changed_at: 2026-08-20T21:31:20Z
superseded_by: null
```

GOAL: Failover heartbeat LaunchAgent running again after cutover disarm.

Set `status:` above to `filed` or `skipped` once reviewed (the sweep reads your edit back and never re-drafts mesh_done_card rows it has already seen).

## notes
