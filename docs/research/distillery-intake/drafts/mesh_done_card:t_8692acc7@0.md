# HUMAN: land the 3 crucible docs + campaign-ledger row

```yaml
id: mesh_done_card:t_8692acc7@0
source_kind: mesh_done_card
source_path: t_8692acc7
source_ref: t_8692acc7@0
board: mesh
captured_at: 2026-08-09T03:15:16Z
status: draft
status_changed_at: 2026-08-09T03:15:16Z
superseded_by: null
```

Sprint: S0-preserve. Unblock: Ryan copies from chat (scratchpad is session-ephemeral).

Set `status:` above to `filed` or `skipped` once reviewed (the sweep reads your edit back and never re-drafts mesh_done_card rows it has already seen).

## notes
