...
# ---------------------------------------------------------------------------
# Estimate — a rough token/complexity/local-vs-frontier estimate for a task,
# via the auxiliary (auto-routed) model. Core logic lives in
# hermes_cli.kanban_estimate (shared with the gateway dispatcher's
# auto-estimate-on-create tick — see kanban_db.estimate_status / t_5716d9c5)
# so the on-click path here and the on-create path there cannot drift.
# ---------------------------------------------------------------------------

from hermes_cli.kanban_estimate import run_estimate as _run_estimate  # noqa: E402
...
def estimate_task_endpoint(task_id: str, board: Optional[str] = Query(None)):
    """Rough token + complexity + local/frontier lane estimate for an existing
    task via the auxiliary model. Returns
    ``{ok, est_tokens, complexity, lane, rationale, model}``; a non-OK
...
def _default_workspace_kind(board: dict[str, Any]) -> str:
    ...
    try:
        return kanban_db.resolve_board_default_workspace_kind(meta=board)
    except Exception:
